...
import { Eye, Edit, Users, Building, MapPin, Clock, DollarSign, Calendar, User, MessageSquare, Mail, Phone, Plus, X, TrendingUp, Briefcase, FileText, Bell, Globe, Mail as MailIcon, Phone as PhoneIcon, Linkedin, Github } from 'lucide-react';
...
export default function CompanyDashboard() {
  ...
  const candidatos = [
    {
      ...
      mensaje: 'Me interesa mucho esta posición. Tengo 3 años de experiencia en React y Node.js.',
      fecha: 'Hace 2 días',
      ofertaId: 1,
      ...
      habilidades: ['React', 'Node.js', 'JavaScript', 'TypeScript', 'MongoDB', 'PostgreSQL'],
    ...
    },
    {
      ...
      mensaje: 'Perfecto para mi perfil. He trabajado con las tecnologías que mencionan.',
      fecha: 'Hace 1 día',
      ofertaId: 1,
      // Información adicional del perfil
      edad: 25,
      ...
      experiencia: '2 años',
      educacion: 'Grado en Desarrollo de Aplicaciones Web - Universitat Pompeu Fabra',
    ...
    },
    {
      ...
      mensaje: 'Me encantaría formar parte del equipo. Tengo experiencia en Python y Django.',
      fecha: 'Hace 3 días',
      ofertaId: 2,
      // Información adicional del perfil
      edad: 32,
      ...
      preferencias: ['Presencial', 'Liderazgo técnico', 'Arquitectura de software']
    }
  ];
  const mensajes = [
    {
  ...
  const estadisticas = {
    ...
    ofertasActivas: ofertas.filter(o => o.timePosted.includes('día') || o.timePosted.includes('hora')).length,
    candidatosRecientes: candidatos.filter(c => c.fecha.includes('día')).length,
    tasaRespuesta: Math.round((mensajes.length / candidatos.length) * 100) || 0
  };

  // Datos para el chart de candidatos por oferta
  const chartData = ofertas.map(oferta => ({
    oferta: oferta.title,
    candidatos: candidatos.filter(c => c.ofertaId === oferta.id).length
  }));

  const mostrarCandidatos = (ofertaId) => {
    const candidatosFiltrados = candidatos.filter(c => c.ofertaId === ofertaId);
  ...
  const guardarOferta = () => {
    ...
    console.log('Guardando oferta:', formOferta);
    setCreandoOferta(false);
    setEditandoOferta(false);
    setFormOferta({
      title: '',
  ...
  useEffect(() => {
    ...
    if (candidatoParam && seccionParam === 'mensajes') {
      try {
        ...
        const conversacion = mensajes.find(m => 
      ...
      <main className="flex-1 p-8">
        {seccion === 'perfil' && (
          <>
            ...
            <div className="mb-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <Card className="p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-2xl font-bold text-gray-900">{estadisticas.totalOfertas}</p>
                ...
                <Card className="p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-2xl font-bold text-gray-900">{estadisticas.totalCandidatos}</p>
                      <p className="text-xs text-green-600 flex items-center mt-1">
                        <TrendingUp className="w-3 h-3 mr-1" />
                ...
                <Card className="p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-2xl font-bold text-gray-900">{estadisticas.mensajesNoLeidos}</p>
                ...
                <Card className="p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-sm font-medium text-gray-600">Ofertas Remotas</p>
                      <p className="text-2xl font-bold text-gray-900">
                        {ofertas.filter(o => o.isRemote).length}
                      </p>
            ...
            <div className="mb-8">
              <Card className="p-6">
                <h3 className="text-lg font-semibold mb-4">Candidatos por oferta</h3>
                    ...
                    <div key={index} className="flex items-center gap-4">
                      <div className="flex-1">
                        <div className="flex justify-between text-sm mb-1">
                          ...
                          <span className="text-gray-600">{item.candidatos} candidatos</span>
                        </div>
                        <div className="w-full bg-gray-200 rounded-full h-2">
                          <div 
                            className="bg-blue-600 h-2 rounded-full transition-all duration-300"
                            style={{ width: `${(item.candidatos / Math.max(...chartData.map(d => d.candidatos))) * 100}%` }}
                          ></div>
                        </div>
                      </div>
                    </div>
                  ...
                  {!editandoPerfil ? (
                    <>
                      <div className="flex justify-between items-start mb-6">
                        <div>
                          <h2 className="text-2xl font-bold mb-2">{empresa.nombre}</h2>
                          <p className="text-gray-600">{empresa.descripcion}</p>
                        </div>
                        <Button onClick={() => setEditandoPerfil(true)} variant="outline">
                        ...
                        <div className="space-y-4">
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-gray-900">{empresa.telefono}</p>
                            </div>
                          </div>
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-gray-900">{empresa.ubicacion}</p>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="space-y-4">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-gray-900">{empresa.linkedin || 'No especificado'}</p>
                            </div>
                          </div>
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-gray-900">{empresa.github || 'No especificado'}</p>
                            </div>
                          </div>
                    ...
                    <>
                      <div className="flex justify-between items-start mb-6">
                        <h2 className="text-2xl font-bold">Editar perfil de la empresa</h2>
                      ...
                      <form className="space-y-4">
                        <div>
                          ...
                          <Input type="text" placeholder="Nombre de la empresa" defaultValue={empresa.nombre} />
                        </div>
                        <div>
                          <Label>Descripción</Label>
                          <Textarea placeholder="Describe tu empresa..." defaultValue={empresa.descripcion} />
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div>
                            <Label>Email</Label>
                            <Input type="email" placeholder="Correo de contacto" defaultValue={empresa.email} />
                          </div>
                          <div>
                            <Label>Teléfono</Label>
                            <Input type="tel" placeholder="Teléfono" defaultValue={empresa.telefono} />
                          </div>
                        </div>
                        <div>
                          <Label>Ubicación</Label>
                          <Input type="text" placeholder="Ubicación" defaultValue={empresa.ubicacion} />
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div>
                            <Label>LinkedIn</Label>
                            <Input type="url" placeholder="Enlace a LinkedIn" defaultValue={empresa.linkedin} />
                          </div>
                          <div>
                            <Label>GitHub</Label>
                            <Input type="url" placeholder="Enlace a GitHub" defaultValue={empresa.github} />
                          </div>
                        </div>
                        <div className="flex gap-2">
                          <Button type="submit">Guardar cambios</Button>
                          <Button type="button" variant="outline" onClick={() => setEditandoPerfil(false)}>Cancelar</Button>
                        </div>
                      </form>
                    </>
                  )}
                ...
                <Card className="p-6">
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1">
                        <p className="text-sm font-medium">Nueva oferta publicada</p>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1">
                        <p className="text-sm font-medium">5 candidatos aplicaron</p>
            ...
            {!creandoOferta && !editandoOferta ? (
              <>
                <div className="flex justify-between items-center mb-6">
                  <h2 className="text-2xl font-bold">Ofertas de trabajo publicadas</h2>
                  <Button onClick={abrirFormularioCreacion}>Crear nueva oferta</Button>
                </div>
                <div className="space-y-4">
                  {ofertas.map((oferta) => (
                    <div
                    ...
                    >
                      <div className="flex items-start justify-between mb-3">
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            <h3 className="font-semibold text-gray-900">{oferta.title}</h3>
                            <p className="text-sm text-gray-600">{oferta.company}</p>
                          </div>
                        </div>
                      ...
                      <div className="flex flex-wrap gap-2 mb-3">
                        <span className={`px-3 py-1 rounded-full text-xs font-medium ${
                          oferta.type === 'Full Time' ? 'bg-orange-100 text-orange-800' : 'bg-gray-100 text-gray-800'
                        }`}>
                          {oferta.type}
                        </span>
                        {oferta.isRemote && (
                ...
                <Dialog open={!!ofertaSeleccionada} onOpenChange={() => setOfertaSeleccionada(null)}>
                    ...
                    {ofertaSeleccionada && (
                      <div className="space-y-4">
                        <div className="flex items-start justify-between">
                          <div>
                            <h2 className="text-2xl font-bold text-gray-900 mb-2">{ofertaSeleccionada.title}</h2>
                            <div className="flex items-center space-x-4 text-gray-600 mb-4">
                              <span className="flex items-center">
                                <Building className="w-4 h-4 mr-1" />
                                {ofertaSeleccionada.company}
                              </span>
                              <span className="flex items-center">
                                <MapPin className="w-4 h-4 mr-1" />
                                {ofertaSeleccionada.location}
                              </span>
                              <span className="flex items-center">
                                <Clock className="w-4 h-4 mr-1" />
                        ...
                        <div className="flex items-center space-x-4">
                          <span className="flex items-center text-gray-600">
                            <Building className="w-4 h-4 mr-1" />
                            {ofertaSeleccionada.type} • {ofertaSeleccionada.level}
                          </span>
                          <span className="flex items-center text-gray-600">
                            <Users className="w-4 h-4 mr-1" />
                        ...
                        <div className="border-t pt-6">
                          <h3 className="text-lg font-semibold text-gray-900 mb-4">Descripción del puesto</h3>
                ...
                <Dialog open={candidatosDialog} onOpenChange={() => setCandidatosDialog(false)}>
                        ...
                        <Card key={candidato.id} className="p-4">
                          <div className="flex items-start justify-between mb-3">
                            <div>
                              ...
                              <p className="text-sm text-gray-600">{candidato.telefono}</p>
                            </div>
                            <span className="text-xs text-gray-500">{candidato.fecha}</span>
                          </div>
                          <div className="mb-3">
                            <h4 className="font-medium text-sm mb-1">Descripción:</h4>
                            <p className="text-sm text-gray-700">{candidato.descripcion}</p>
                          </div>
                          <div className="bg-blue-50 p-3 rounded-lg">
                            <h4 className="font-medium text-sm mb-1">Mensaje de aplicación:</h4>
                            <p className="text-sm text-gray-700">{candidato.mensaje}</p>
                          </div>
              ...
              <div className="max-w-4xl mx-auto">
                <div className="flex justify-between items-center mb-6">
                  <h2 className="text-2xl font-bold">
                    {editandoOferta ? 'Editar oferta de trabajo' : 'Crear nueva oferta de trabajo'}
                  </h2>
                ...
                <Card className="p-8">
                  <form onSubmit={(e) => { e.preventDefault(); guardarOferta(); }} className="space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div>
                    ...
                    <div>
                      ...
                      <div className="space-y-3">
                        <div className="flex gap-2">
                          ...
                          />
                          <Button type="button" onClick={agregarSkill} variant="outline">
                    ...
                    <div className="flex gap-3 pt-4">
                      <Button type="submit" className="flex-1">
                        {editandoOferta ? 'Actualizar oferta' : 'Publicar oferta'}
                      </Button>
                      <Button
                        type="button"
                        variant="outline"
                  ...
                  {mensajes.map((mensaje) => (
                    <div
                      ...
                      className={`p-4 border-b cursor-pointer hover:bg-gray-50 transition-colors ${
                        conversacionSeleccionada?.id === mensaje.id ? 'bg-blue-50 border-blue-200' : ''
                      }`}
                    >
                      <div className="flex items-start justify-between mb-2">
                        <div className="flex-1">
                          <h4 className="font-medium text-sm">{mensaje.candidato}</h4>
                          <p className="text-xs text-gray-500">{mensaje.oferta}</p>
                        </div>
                        {!mensaje.leido && (
                          <div className="w-2 h-2 bg-blue-500 rounded-full"></div>
                        )}
                      </div>
                      <p className="text-sm text-gray-600 truncate">{mensaje.ultimoMensaje}</p>
                ...
                {conversacionSeleccionada ? (
                  <>
                    <div className="p-4 border-b">
                      <div className="flex items-center justify-between">
                        <div>
                          <h3 className="font-semibold">{conversacionSeleccionada.candidato}</h3>
                          <p className="text-sm text-gray-600">{conversacionSeleccionada.oferta}</p>
                          <p className="text-xs text-gray-500">{conversacionSeleccionada.email}</p>
                        </div>
                        <div className="flex gap-2">
                    ...
                    <div className="h-[500px] overflow-y-auto p-4">
                      <div className="space-y-4">
                        {conversacionSeleccionada.conversacion.map((msg) => (
                          <div
                            key={msg.id}
                            className={`flex ${msg.autor === 'empresa' ? 'justify-end' : 'justify-start'}`}
                          >
                            <div
                            ...
                            >
                              ...
                              <p className={`text-xs mt-1 ${
                                msg.autor === 'empresa' ? 'text-blue-100' : 'text-gray-500'
                              }`}>
                                {msg.fecha}
                              </p>
                            </div>
